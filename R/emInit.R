...
emInit <- function(y, u, g, conds, lib.size, lib.type, alg.type = "EM",
	starts = 5, fixed.lambda, equal.proportions,
	verbose) {
	...
	for(start in 1:starts) {
		em.init <- PoisMixClus(y = y, u = u, g = g, lib.size = lib.size,
			lib.type = lib.type, conds = conds,
			init.type = init.type1, alg.type = alg.type, iter = 5,
