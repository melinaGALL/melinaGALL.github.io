...
PoisMixClus <- function(y, u, g, conds, lib.size = TRUE, lib.type = "TMM",
	init.type = "small-em", init.runs = 5, alg.type = "EM", cutoff = 10e-6, iter = 1000,
	fixed.lambda = NA, equal.proportions = FALSE, prev.labels = NA,
	prev.probaPost = NA, verbose = FALSE) {
	...
	if(init.type == "small-em") {
		init.alg <- "emInit"
		init.args <- list(y = y, u = u, g = g, conds = conds, lib.size = lib.size,
			lib.type = lib.type, alg.type = alg.type, starts = 5,
	...
	if(init.type == "split.small-em") {
		...
		init.args <- list(y = y, g = g, conds = conds, lib.size = lib.size,
			...
			init.runs = init.runs, verbose = verbose)
	}
	...
	if(min(pi) > 0 | is.nan(sum(lambda)) == FALSE) {
		...
		labels <- map
                ...
                #####################################
                ## Calulate f(y|z) for MIL
                #####################################
                lambda.MIL <- lambda
                denom.MIL <- colSums(z * w)
                for(j in 1:d) {
                    denom.bis.MIL <- denom.MIL * s.dot[j]
                    num.MIL <- colSums(z * matrix(rep(rowSums(as.matrix(y[,which(conds == (unique(conds))[j])])),K),ncol = K))
                    lambda.MIL[j,] <- num.MIL / denom.bis.MIL
				}
                mean.calc.MIL <- PoisMixMean(y, g = K, conds, s, lambda.MIL)
                p.LL.tmp <- logLikePoisMixMIL(y, mean.calc.MIL, pi, labels)
		p.LL <- p.LL.tmp$ll


                ####################################
                ## Calculate entropy term for SICL, MIL
                #####################################
                ##u <- as.matrix(u)
                entropyS <- entropySupCal(map, u)
                entropyM <- entropyMarCal(map, u)

		##############################
		## Calculate BIC, ICL, SICL, MIL
		##############################
		...
		ICL <- BIC + entropy
                SICL <- BIC + entropy - entropyS
                MIL <- -p.LL + (n + (d-1)*K)/2*log(n) -  entropyM
	}
	results <- list(lambda = lambda.final, pi = pi.final, labels = labels,
		probaPost = probaPost, log.like = LL, p.log.like = p.LL,
                entropy = entropy, entropyM = entropyM, entropyS = entropyS,
                BIC = -BIC, ICL = -ICL, SICL = -SICL, MIL = -MIL,
		alg.type = alg.type, lib.size = lib.size, lib.type = lib.type,
                s = s, conds = conds)
